...
class Transformer(t2t_model.T2TModel):
  ...
  def decode(self,
    ...
    decoder_output = transformer_decoder(
        ...
        cache=cache,
        name=name,
        reuse_common=reuse_common,
        reuse_crosslingual_attn=reuse_crosslingual_attn
    )
  ...
  with tf.variable_scope(name):
    for layer in xrange(hparams.num_decoder_layers or
      ...
      with tf.variable_scope(layer_name):
        with tf.variable_scope("self_attention", reuse=reuse_common):
          y = common_attention.multihead_attention(
              common_layers.layer_preprocess(x, hparams),
              None,
              decoder_self_attention_bias,
        ...
        if encoder_output is not None:
          with tf.variable_scope("encdec_attention", reuse=reuse_crosslingual_attn):
            ...
            y = common_attention.multihead_attention(
                ...
                hparams.attention_dropout)
            x = common_layers.layer_postprocess(x, y, hparams)
        with tf.variable_scope("ffn", reuse=reuse_common):
          y = transformer_ffn_layer(
    ...
    # on the output, since the output can grow very large, being the sum of
    # a whole stack of unnormalized layer outputs.
  with tf.variable_scope(name, reuse=reuse_common):
    return common_layers.layer_preprocess(x, hparams)
