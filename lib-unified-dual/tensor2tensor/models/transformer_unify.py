...
class Transformer_Unify(Transformer):
  ...
  def _beam_decode(self, features, decode_length, beam_size, top_beams,
    ...
    decoded_ids, scores = self._fast_decode(
        features, decode_length, beam_size, top_beams, alpha, use_rev)
  ...
  def _fast_decode(self,
    ...
    with tf.variable_scope("body"):
      if use_rev:      
        tgtSpaceID = features["input_space_id"]
      else:
        tgtSpaceID = features["target_space_id"]
      encoder_output, encoder_decoder_attention_bias = dp(
            self.omni_encoder, inputs, tgtSpaceID, hparams, encoder_name)
